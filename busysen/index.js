...
app.post("/rooms/:id/reservations", async function (req, res) {
  ...
  await newReservation(client,{
    ...
    reservations : {
      ...
      idClient: req.session.username,
      _id: new BSON.ObjectId()
    }});
  res.status(200).json("success")
})

app.get('/rooms/:room', async function (req, res) {

  const room = req.params.room
  let result = await getDataRoom(client, room);
  res.status(200).json(result)
});

app.get('/levels/max', async function (req, res) {
  res.status(200).json(7+ await getLevelMax(client))
});



app.get('/levels/custom/:id', async function (req, res) {

  const id = parseInt(req.params.id)
  let room = await getRoomCustom(client, id);
  let level = await getLevel(client,id);
  res.status(200).json({level:level,room:room})
});

app.get('/reservations', async function (req, res) {
  let reservations = await getDataUser(client, req.session.username);
...
app.delete('/reservations', async function (req, res) {
  const nameRoom = req.query.room
  const idReservations = req.query.id
  await removeReservation(client, nameRoom, idReservations);
  res.status(200).json("success")
});
...
http.listen(4202, () => {
  console.log('Serveur lancé sur le port 4202');
});
...
async function getLevelMax(client) {
  return await client.db("Projet-Info").collection("CustomLevels").count();
}
